...
if(isset($_POST['search']) && $_POST['search']!='' ) {
  ...
  if(isset($start_date) && $start_date!='' && isset($end_date) && $end_date!='' ) {
    $statement = "`$table` WHERE DATE_FORMAT(date,'%Y-%m-%d') between '$from_change_format' AND '$to_change_format'";
    ...
    $statement = "`$table` WHERE  DATE_FORMAT(date,'%Y-%m-%d') = '$from_change_format'";
    //echo "SELECT * FROM {$statement} "; 
    $getData = $conn->query("SELECT * FROM {$statement} ");
  } elseif(isset($start_date) && $start_date!='' ) {
    $statement = "`$table` WHERE DATE_FORMAT(date,'%Y-%m-%d') = '$from_change_format'";
    ...
    $sql="select * from `truck_maintance`";
...
<center><table>
  ...
  <?php   
        $i=1; 
        $maintance_category_name = array(); 
        $total_ltrs = 0;
        while ($row = $getData->fetch_assoc()) {           
        $maintance_category_name[] = serialize($row['maintance_category_name']);        

  ?>
  <tr>
    <td><?php echo $i; ?></td>
    <td><?php $getVendorName = getIndividualDetails($row['maintance_category_name'],'maintance_category','id'); echo $getVendorName['title']; ?></td> 
    <td><?php echo $row['price']; ?></td>
    <td><?php echo $row['description']; ?></td>    
    <!-- <td> <a href="TCPDF/examples/view_vendor_milk_pdf.php?uid=<?php echo $row['maintance_category_name']; ?>" target="_blank">Print</a></td> -->
  </tr>
  <?php $i++;  } ?>
</table>
  <?php 
...
<div class="container-fluid footer">
    <center><a href="TCPDF/examples/monthly_truck_pdf_reports.php?start_date=<?php echo $start_date; ?>&end_date=<?php echo $end_date; ?>" target="_blank" class="btn btn-default btn-lg" style="background-color:#35b863; color:white">Generate Reports</a></center>
</div>
</body>
</html>
<script>
